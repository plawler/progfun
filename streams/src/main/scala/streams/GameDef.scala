...
trait GameDef {
  ...
  case class Block(b1: Pos, b2: Pos) {
     ...
     */
    def neighbors: List[(Block, Move)] = List((up, Up), (down, Down), (right, Right), (left, Left))
     ...
     */
    def isLegal: Boolean = terrain(b1) && terrain(b2) // remember that a Terrain returns a function that takes a Pos and returns a Boolean

    def isDone: Boolean = isStanding && b1 == goal
